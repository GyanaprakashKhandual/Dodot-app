...
export default function TodoDashboard() {
  ...
  const updateTodo = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setTodos(
          todos.map((todo) =>
            todo._id === editingTodo._id ? { ...todo, ...formData } : todo
          )
        );
  ...
  const markAsComplete = (id) => {
    setConfirmModal({
      ...
      message:
        "Congratulations! Are you sure you want to mark this todo as completed?",
      type: "success",
      onConfirm: () => {
        setTodos(
          todos.map((todo) =>
            todo._id === id ? { ...todo, status: "Working" } : todo
          )
        );
  ...
  const filteredTodos = todos.filter((todo) => {
    ...
    const matchesWorkType =
      filterWorkType === "all" || todo.workType === filterWorkType;
    const matchesSearch = todo.workName
      .toLowerCase()
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 mb-6 sm:mb-8">
          {statuses.map((status) => {
            ...
            return (
              <div
                key={status}
                className="bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 transition-colors duration-300"
              >
